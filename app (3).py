    ...
    if app_mode == "📊 Executive Dashboard":
        ...
        if not maint_df.empty:
            ...
            if (
                ...
                st.subheader(
                    "✏️ Edit Service Event"
                )

                edit_options = {
                    f"{r.get('service_date','')} "
                    f"• {r.get('registration','')} "
                    f"• {r.get('service_type','')}":
                        r["id"]
                    for _, r
                    in maint_df.iterrows()
                }
                ...
                if not edit_matches.empty:
                    ...
                    with st.form(
                        "edit_maintenance_form"
                    ):
                        c1, c2 = st.columns(2)

                        e_service_date = c1.date_input(
                            "Service Date",
                            value=default_service_date
                        )

                        e_registration = c2.text_input(
                            "Registration",
                            value=str(
                                er.get("registration") or ""
                            )
                        ...
                        e_odo_service = c5.number_input(
                            "Odo at Service",
                            min_value=0.0,
                            value=numeric_value(
                                er.get("odo_at_service")
                            )
                        )

                        e_next_service = c6.number_input(
                            "Next Service KM",
                            min_value=0.0,
                            value=numeric_value(
                                er.get("next_service_km")
                        ...
                        if st.form_submit_button(
                            ...
                            try:
                                ...
                                st.error(
                                    f"Could not update service event: {e}"
                                )
            ...
            if (
                ...
                selected = st.selectbox(
                    "Service Event",
                    list(options.keys()),
                    key="delete_maint_select"
                )

                confirm = st.checkbox(
                    "I understand this permanently deletes the selected service event.",
                    key="delete_maint_confirm"
                )

                if st.button(
                    "🗑️ Delete Selected Service Event",
                    disabled=not confirm,
                    key="delete_maint_button"
                ):
                    try:
                        old = delete_scoped_record(
                            client,
                        ...
                        st.error(
                            f"Could not delete service event: {e}"
                        )
        ...
        if not gps.empty:
            ...
            if (
                ...
                if not edit_matches.empty:
                    ...
                    with st.form(
                        ...
                        e_log_date = c1.date_input(
                            "Date",
                            value=default_log_date
                        )
                        ...
                        e_idle = c10.number_input(
                            "Idle Time (min)",
                            0.0,
                            value=numeric_value(
                                er.get("idle_time_min")
                            )
                        )

                        e_fuel_start = c11.number_input(
                            "Fuel Start (%)",
                            0.0,
                            100.0,
                            value=numeric_value(
                                er.get("fuel_level_start"),
                                100.0
                            )
                        )

                        e_fuel_end = c12.number_input(
                            "Fuel End (%)",
                            0.0,
                            100.0,
                            value=numeric_value(
                                er.get("fuel_level_end")
                            )
                        )

                        e_trip_distance = st.number_input(
                            "Trip Log Distance",
                            0.0,
                            value=numeric_value(
                                er.get("trip_log_distance")
                            )
                        )

                        if st.form_submit_button(
                            "💾 Save GPS Record Changes"
                        ):
                            fields = {
                                "log_date":
                                    str(e_log_date),
                            ...
                            try:
                                ...
                                st.error(
                                    f"Could not update GPS record: {e}"
                                )
            ...
            if (
                ...
                selected = st.selectbox(
                    "GPS Record",
                    list(options.keys()),
                    key="delete_gps_select"
                )

                confirm = st.checkbox(
                    "I understand this permanently deletes the selected GPS record.",
                    key="delete_gps_confirm"
                )

                if st.button(
                    "🗑️ Delete Selected GPS Record",
                    disabled=not confirm,
                    key="delete_gps_button"
                ):
                    try:
                        old = delete_scoped_record(
                            client,
                        ...
                        st.error(
                            f"Could not delete GPS record: {e}"
                        )
